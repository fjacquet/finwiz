...
# CrewAI Best Practices for FinWiz

> Comprehensive guide to CrewAI patterns for building resilient, efficient flows and crews

## Table of Contents

1. [Flow State Management](#flow-state-management)
2. [Agent Reasoning](#agent-reasoning)
3. [Crew Planning](#crew-planning)
...
### ✅ ALWAYS Use Structured Pydantic State
...
class MyFlow(Flow[MyFlowState]):  # Type parameter required
    @start()
    def initialize(self):
        self.state.holdings_processed = 0  # Type-safe access
        return "Initialized"
```

### ❌ NEVER Use Unstructured Dict State
...
class BadFlow(Flow):
    @start()
    def initialize(self):
        self.state["holdings_processed"] = 0  # Unstructured
        return "Initialized"
```
...
@persist()  # Automatic persistence
class PersistentFlow(Flow[MyState]):
    @start()
    def step_one(self):
        self.state.value += 1
        return "Done"  # State saved automatically
```
...
class ResumableFlow(Flow[MyState]):
    ...
    @start("init")  # Conditional - runs after init OR on resume
...
### State-Based Routing with @router
...
### Reasoning with Retry Logic

```python
@retry(stop=stop_after_attempt(3))
def analyze_with_reasoning(ticker: str, attempt: int = 1):
    # Adjust reasoning attempts based on retry count
    max_attempts = max(1, 4 - attempt)  # Fewer on retries
    ...
    failure_context = ""
    if attempt > 1:
        failure_context = f"\nPrevious {attempt-1} attempts failed."
    ...
    task = Task(
        ...
        Focus on: data validation, error handling, comprehensive analysis
        """,
        agent=analyst
    )
...
- **Cost**: Minimal for occasional use

---

## Crew Planning

### ❌ NOT RECOMMENDED for High-Volume Single-Agent Crews

```python
# ❌ BAD - Planning for high-volume single-agent crew
class DeepAnalysisCrew(CrewBase):
    ...
    def crew(self) -> Crew:
        return Crew(
            agents=self.agents,
            tasks=self.tasks,
            planning=True,  # ❌ Overhead for 66 executions
            planning_llm="gpt-5-mini"
        )
```

**Why not:**
- 66 holdings × planning overhead = significant cost
- Single-agent crew doesn't need coordination
- Agent reasoning is sufficient
- Additional API calls per execution

### ✅ RECOMMENDED for Multi-Agent Coordination

```python
# ✅ GOOD - Planning for complex multi-agent crew
...
| Crew | Agents | Tasks | Volume | Planning? | Reason |
|------|--------|-------|--------|-----------|--------|
| DeepAnalysisCrew | 3 | 4 | High (66×) | ❌ No | Use agent reasoning |
| PortfolioRebalancingCrew | 4+ | 6+ | Low (1×) | ✅ Yes | Complex coordination |
| InvestmentDiscoveryCrew | 4+ | 5+ | Low (3×) | ✅ Yes | Multi-agent screening |
| ReportCrew | 1 | 1 | Low (1×) | ❌ No | Single consolidation |
...
```python
# ✅ RECOMMENDED - Selective delegation
class DeepAnalysisCrew(CrewBase):
    @agent
...
- Agents that need to ask questions
...
- **Time**: 5-15 seconds per delegation
- **API Calls**: 1-2 additional LLM calls
- **Cost**: Minimal for occasional use
- **For 66 holdings**: ~6-7 collaborations if 10% delegate
...
- [ ] Consider volume (how many executions)
- [ ] Calculate cost (planning overhead × execution count)
- [ ] Test with planning enabled/disabled
- [ ] Monitor performance impact
- [ ] Document decision (why enabled/disabled)
- [ ] Configure planning LLM if enabled
...
### Flow State
1. **Always use structured Pydantic state** - Type safety prevents errors
2. **Use @persist() for checkpointing** - Native CrewAI persistence
3. **Conditional @start() for resume** - Built-in resume capability
4. **Return data from methods** - Enable downstream listeners
5. **Use @router for branching** - State-based conditional logic

### Agent Reasoning
1. **Enable for complex tasks** - Planning improves reliability
...
### Crew Planning
1. **Not for high-volume** - Overhead multiplies with executions
2. **Use for multi-agent coordination** - Complex workflows benefit
3. **OpenAI key required** - Default planning uses gpt-5-mini
4. **Agent reasoning preferred** - For single-agent crews
5. **Test and measure** - Compare with/without planning

### Agent Collaboration
1. **Selective delegation** - Coordinators yes, specialists maybe not
2. **Minimal overhead** - 5-15 seconds per collaboration
3. **Context passing works** - Task dependencies often sufficient
4. **Clear roles matter** - Specific roles reduce confusion
5. **Monitor and measure** - Track collaboration patterns
...
- CrewAI Agent Reasoning Documentation
- CrewAI Crew Planning Documentation
- CrewAI Agent Collaboration Documentation
- FinWiz existing patterns: `flow_orchestrator.py`, `deep_analysis.py`

---

**Version**: 1.0  
**Created**: 2025-01-11  
**Purpose**: Comprehensive CrewAI best practices for FinWiz development
...
**Version**: 1.0  
**Created**: 2025-01-08  
**Purpose**: Capture critical lessons from deep portfolio analysis Flow integration# CrewAI Development Standards for FinWiz
...
# Flow Architecture Lessons Learned
...
### Problem Discovered
The original flow had discovery crews (check_crypto, check_stock, check_etf) running BEFORE portfolio analysis. This is backwards from the logical business process.
...
This listener waited for `match_alternatives` but NOT for `update_portfolio_review_with_deep_analysis`, which also listened to `match_alternatives`. This created a race condition where discovery could start before portfolio was updated.
