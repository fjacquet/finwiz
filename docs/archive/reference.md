...
- `AlternativeFinder`: Find better alternatives for underperforming holdings with A+ integration:
  - **A+ Prioritization**: Integrates with investment discovery crew A+ outputs
  - **Smart Matching**: Finds alternatives based on asset class, sector, and risk profile
  - **Transition Strategies**: Provides immediate, gradual, or tax-optimized swap timing
  - **Asset-Specific Metrics**: Expense ratio savings (ETFs), fundamental improvements (stocks), liquidity improvements (crypto)
  - **Grade Improvements**: Calculates expected grade and score improvements
  - **French Output**: All recommendations and rationale in French
  - **Tax Awareness**: Includes tax implications for each transition strategy
  - Only suggests alternatives for holdings graded below B (C+, C, D, F)
  - Limits to 3 alternatives per holding with duplicate removal
- `PriceTargetCalculator`: Calculate actionable buy/sell price targets for holdings:
  - **Fair Value Analysis**: DCF, P/E multiples, and NAV-based valuations
  - **Technical Levels**: Support/resistance detection from price history
  - **Multi-Asset Support**: Stocks, ETFs, and crypto with asset-specific logic
  - **Buy Targets**: Primary and secondary accumulation levels with rationale
  - **Sell Targets**: Profit-taking and stop-loss levels with risk management
  - **Multi-Currency**: Native currency price targets with FX risk notes
  - **Confidence Scoring**: 0-100% confidence based on data availability
  - **French Rationale**: All explanations in French for user clarity
- `HoldingAnalyzerOrchestrator`: Coordinate deep analysis across stock/ETF/crypto crews:
  - **Crew Integration**: Leverages existing crew outputs with 7-day caching
  - **Schema Mapping**: Maps crew-specific data to portfolio review schema
  - **Fallback Logic**: Graceful degradation when crew analysis unavailable
  - **Parallel Processing**: Analyzes multiple holdings concurrently
