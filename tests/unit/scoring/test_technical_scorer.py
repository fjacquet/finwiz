...
class TestTechnicalScorer:
    ...
    def test_technical_score_strong_uptrend(self, scorer):
        ...
        assert details["rsi_score"] == 1.0
        assert details["trend_score"] == 1.0
        assert details["trend_direction"] == "strong_uptrend"
        assert details["momentum_score"] == 1.0
    ...
    def test_technical_score_strong_downtrend(self, scorer):
        ...
        assert details["rsi_score"] == 0.2
        assert details["trend_score"] == 0.2
        assert details["trend_direction"] == "strong_downtrend"
        assert details["momentum_score"] == 0.4  # Bearish (not strong bearish)
    ...
    def test_rsi_scoring_ranges(self, scorer):
        ...
        data["rsi"] = 35
        score, details = scorer.calculate_technical_score(data)
        assert details["rsi_score"] == 0.8

        # Acceptable range (20-80)
        data["rsi"] = 75
        score, details = scorer.calculate_technical_score(data)
        assert details["rsi_score"] == 0.6

        # Warning range (10-90)
        data["rsi"] = 85
        score, details = scorer.calculate_technical_score(data)
        assert details["rsi_score"] == 0.4
    ...
    def test_trend_direction_detection(self, scorer):
        ...
        assert details["trend_score"] == 0.8
        ...
        assert details["trend_score"] == 0.6
        ...
        score, details = scorer.calculate_technical_score(data)
        assert details["trend_direction"] == "sideways"
        assert details["trend_score"] == 0.5
