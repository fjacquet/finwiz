...
class TestETFAnalyzer:
    ...
    def test_calculate_fundamental_score_poor_etf(self, analyzer, poor_etf_data):
        ...
        assert score <= 0.3
        assert details["fundamental_score"] == score
        assert details["expense_score"] == 0.2
        assert details["tracking_score"] == 0.2
    ...
    def test_score_expense_ratio_thresholds(self, analyzer):
        ...
        assert analyzer._score_expense_ratio(0.0005) == 1.0  # Excellent
        assert analyzer._score_expense_ratio(0.002) == 0.8  # Very good
        assert analyzer._score_expense_ratio(0.004) == 0.6  # Good
        assert analyzer._score_expense_ratio(0.008) == 0.4  # Acceptable
        assert analyzer._score_expense_ratio(0.02) == 0.2  # Poor

    def test_score_tracking_error_thresholds(self, analyzer):
        """Test tracking error scoring thresholds."""
        assert analyzer._score_tracking_error(0.001) == 1.0  # Excellent
        assert analyzer._score_tracking_error(0.003) == 0.8  # Very good
        assert analyzer._score_tracking_error(0.008) == 0.6  # Good
        assert analyzer._score_tracking_error(0.015) == 0.4  # Acceptable
        assert analyzer._score_tracking_error(0.05) == 0.2  # Poor

    def test_score_aum_thresholds(self, analyzer):
        """Test AUM scoring thresholds."""
        assert analyzer._score_aum(10e9) == 1.0  # $10B+
        assert analyzer._score_aum(2e9) == 0.8  # $2B
        assert analyzer._score_aum(700e6) == 0.6  # $700M
        assert analyzer._score_aum(200e6) == 0.4  # $200M
        assert analyzer._score_aum(50e6) == 0.2  # $50M
