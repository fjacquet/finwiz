...
class TestCryptoAnalyzer:
    ...
    def test_score_market_cap_thresholds(self, analyzer):
        ...
        assert analyzer._score_market_cap(200e9) == 1.0  # Mega cap
        assert analyzer._score_market_cap(50e9) == 0.8  # Large cap
        assert analyzer._score_market_cap(5e9) == 0.6  # Mid cap
        assert analyzer._score_market_cap(500e6) == 0.4  # Small cap
        assert analyzer._score_market_cap(50e6) == 0.2  # Micro cap

    def test_score_volume_thresholds(self, analyzer):
        """Test volume scoring thresholds."""
        assert analyzer._score_volume(15e9) == 1.0  # Very high
        assert analyzer._score_volume(5e9) == 0.8  # High
        assert analyzer._score_volume(500e6) == 0.6  # Good
        assert analyzer._score_volume(50e6) == 0.4  # Moderate
        assert analyzer._score_volume(5e6) == 0.2  # Low

    def test_score_age_thresholds(self, analyzer):
        """Test age scoring thresholds."""
        assert analyzer._score_age(10.0) == 1.0  # Very established
        assert analyzer._score_age(4.0) == 0.8  # Established
        assert analyzer._score_age(2.5) == 0.6  # Maturing
        assert analyzer._score_age(1.5) == 0.4  # Young
        assert analyzer._score_age(0.5) == 0.2  # Very young
