...
class TestWithTimeout:
    ...
    async def test_should_complete_within_timeout(self, mocker):
        ...
        # Act
        result = await with_timeout(fast_operation, timeout_seconds=1, operation_name="Fast operation", value="test")
    ...
    async def test_should_raise_timeout_error_when_exceeded(self, mocker):
        ...
        with pytest.raises(TimeoutError):
            await with_timeout(slow_operation, timeout_seconds=0.1, operation_name="Slow operation")
    ...
    async def test_should_log_start_and_completion(self, mocker):
        ...
        # Act
        await with_timeout(simple_operation, timeout_seconds=1, operation_name="Test operation")
    ...
    async def test_should_use_asyncio_wait_for(self, mocker):
        ...
        # Act
        result = await with_timeout(test_operation, timeout_seconds=5, operation_name="Test")
    ...
    async def test_should_propagate_exceptions_from_coroutine(self, mocker):
        ...
        with pytest.raises(ValueError, match="Operation failed"):
            await with_timeout(failing_operation, timeout_seconds=1, operation_name="Failing operation")
...
class TestWithTimeoutGraceful:
    ...
    async def test_should_log_warning_on_timeout(self, mocker):
        ...
        # Act
        await with_timeout_graceful(slow_operation, timeout_seconds=0.1, operation_name="Slow operation", fallback_value="fallback")
    ...
    async def test_should_use_with_timeout_internally(self, mocker):
        ...
        # Act
        result = await with_timeout_graceful(test_operation, timeout_seconds=5, operation_name="Test", fallback_value="fallback")
...
class TestTimeoutHandlerIntegration:
    ...
    async def test_should_handle_nested_timeouts(self, mocker):
        ...
        async def outer_operation() -> str:
            result = await with_timeout(inner_operation, timeout_seconds=1, operation_name="Inner operation")
            return f"outer_{result}"

        # Act
        result = await with_timeout(outer_operation, timeout_seconds=2, operation_name="Outer operation")
    ...
    async def test_should_handle_complex_return_types(self, mocker):
        ...
        async def complex_operation() -> dict:
            ...
            return {
                "ticker": "AAPL",
                "analysis": {"grade": "A+", "score": 0.95, "metrics": [1, 2, 3]},
                "timestamp": "2025-01-11T12:00:00",
            }

        # Act
        result = await with_timeout(complex_operation, timeout_seconds=1, operation_name="Complex operation")
    ...
    async def test_should_handle_exception_during_timeout(self, mocker):
        ...
        with pytest.raises(RuntimeError, match="Unexpected error"):
            await with_timeout(operation_with_exception, timeout_seconds=1, operation_name="Exception operation")
...
class TestTimeoutHandlerEdgeCases:
    ...
    async def test_should_handle_operation_that_returns_none(self, mocker):
        ...
        # Act
        result = await with_timeout(operation_returning_none, timeout_seconds=1, operation_name="None returning operation")
    ...
    async def test_should_handle_empty_kwargs(self, mocker):
        ...
        # Act
        result = await with_timeout(operation_no_args, timeout_seconds=1, operation_name="No args operation")
    ...
    async def test_should_handle_operation_name_with_special_chars(self, mocker):
        ...
        # Act
        await with_timeout(operation, timeout_seconds=1, operation_name=special_name)
