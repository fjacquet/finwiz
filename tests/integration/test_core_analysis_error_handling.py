...
class TestCoreAnalysisErrorHandlingIntegration:
    ...
    @pytest.fixture
    def mock_crew_classes(self):
        """Mock the crew classes to simulate failures."""
        with (
            patch("finwiz.main.StockCrew") as mock_stock,
            patch("finwiz.main.EtfCrew") as mock_etf,
            patch("finwiz.main.CryptoCrew") as mock_crypto,
        ):
            ...
            mock_crypto_instance.crew.return_value.kickoff.return_value = Mock(raw="Crypto analysis successful")
    ...
    def mock_feature_flags(self):
        ...
        with patch("finwiz.main.is_feature_enabled") as mock_flags:
            mock_flags.return_value = True
            yield mock_flags

    def test_should_continue_execution_when_individual_crews_fail(self, mock_crew_classes, mock_feature_flags):
        ...
        flow.integration_manager.store_crew_output = Mock(return_value=True)
    ...
    def test_should_provide_core_analysis_status_when_crews_fail(self, mock_crew_classes, mock_feature_flags):
    ...
    def test_should_handle_complete_core_analysis_failure(self, mock_feature_flags):
    ...
    def test_should_use_error_handler_for_fallback_strategies(self, mock_crew_classes, mock_feature_flags):
        ...
        flow.error_handler.handle_crew_failure = Mock(return_value=mock_fallback_response)
        flow.integration_manager.store_crew_output = Mock(return_value=True)
    ...
    def test_should_provide_system_health_information(self, mock_crew_classes, mock_feature_flags):
    ...
    def test_should_handle_disabled_crews_gracefully(self):
