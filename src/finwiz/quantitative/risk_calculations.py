...
def calculate_turnover_ratio(rebalancing_result: RebalancingResult) -> float:
    """
    Calculate portfolio turnover ratio.

    Measures the percentage of portfolio that would be traded.

    Args:
        rebalancing_result: Rebalancing analysis result

    Returns:
        Turnover ratio (0-1 scale)
...
def calculate_herfindahl_index(weights: list[float]) -> float:
    ...
    HHI = sum of squared weights. Ranges from 1/n (perfectly diversified)
    to 1 (single asset).
