    ...
    def _get_encryption_key(self) -> str:
        ...
        if not key:
            raise EncryptionError(
                "SUPABASE_ENCRYPTION_KEY environment variable not set. "
                "Encryption is required for sensitive portfolio data."
            )
        
        # Validate key length (minimum 32 characters for security)
        if len(key) < 32:
            raise EncryptionError(
                "SUPABASE_ENCRYPTION_KEY must be at least 32 characters long"
            )
