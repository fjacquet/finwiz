...
class ReportValidator:
    ...
    SUSPICIOUS_TICKERS = [
        "ABC", "XYZ", "LMN", "TEST", "SAMPLE", "EXAMPLE",
        "DEMO", "FAKE", "MOCK", "DUMMY"
    ]
        ...
        for text in self.FORBIDDEN_TEXT:
            if text in html:
                ...
                issues.append(ValidationIssue(
                    ...
                    message=f"Found placeholder text: {text}",
                    location=context
                ))
        
        return issues
    
    def _check_invalid_tickers(
        self,
        html: str,
        validated_tickers: list[str]
        ...
        common_words = {
            "HTML", "HTTP", "HTTPS", "JSON", "API", "URL", "USD", "EUR", "CHF",
            "ETF", "SEC", "VIX", "ROE", "EBIT", "EBITDA", "CEO", "CFO", "CTO",
            "USA", "NYSE", "NASDAQ", "PDF", "CSV", "XML", "SQL", "AWS", "GCP"
        }
...
def validate_report_file(
    ...
    return validator.validate_html_report(
        html_content,
        validated_tickers,
        str(report_path)
    )
