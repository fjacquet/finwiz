...
def _prepare_enriched_context(ticker: str, result: dict[str, Any]) -> dict[str, Any]:
    ...
    composite_score = result.get("composite_score") or result.get("final_score") or quantitative_container.get("composite_score") or 0
    ...
    recommendation = result.get("recommendation") or result.get("final_recommendation") or quantitative_container.get("preliminary_recommendation") or "HOLD"

    # Extract details dicts (these become the metric tables - check nested quantitative first)
    fundamental_metrics = result.get("fundamental_details") or result.get("fundamental_metrics") or quantitative_container.get("fundamental_metrics", {})
    technical_indicators = result.get("technical_details") or result.get("technical_indicators") or quantitative_container.get("technical_indicators", {})
    risk_metrics = result.get("risk_details") or result.get("risk_metrics") or quantitative_container.get("risk_metrics", {})
