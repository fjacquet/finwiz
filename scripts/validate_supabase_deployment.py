...
class DeploymentValidator:
    ...
    async def validate_phase_1(self) -> bool:
        ...
        self.log_check(
            ...
            passed,
            f"Read timeout is {read_timeout}s (expected: >={expected_read}s)"
        )
        ...
        self.log_check(
            ...
            passed,
            f"Write timeout is {write_timeout}s (expected: >={expected_write}s)"
        )
        ...
        self.log_check(
            ...
            passed,
            f"Max retries is {max_retries} (expected: {expected_retries})"
        )
        ...
        self.log_check(
            ...
            passed,
            f"Circuit breaker threshold is {cb_threshold} (expected: >={expected_threshold})"
        )
        ...
        self.log_check(
            ...
            passed,
            f"Circuit breaker timeout is {cb_timeout}s (expected: >={expected_cb_timeout}s)"
        )
        ...
        self.log_check(
            ...
            all_set,
            f"All timeout environment variables configured: {all_set}"
        )
    ...
    async def validate_phase_2(self) -> bool:
        ...
        self.log_check(
            ...
            passed,
            f"Connectivity test timeout is {conn_timeout}s (expected: {expected_conn}s)"
        )
        ...
        self.log_check(
            ...
            passed,
            f"Test completed in {elapsed:.2f}s (timeout: {conn_timeout}s), result: {connectivity_result}"
        )
        ...
        try:
            ...
            result, is_cached = await self.cache_service.get_or_execute(
                ticker="TEST",
                asset_class="stock",
            ...
            self.log_check(
                ...
                passed,
                f"Analysis completed with cache disabled: {passed}"
            )
        finally:
            # Restore cache state
        ...
        self.log_check(
            "Availability Flag",
            passed,
            f"Client has is_available flag: {passed}, value: {getattr(self.client, 'is_available', None)}"
        )
    ...
    async def validate_success_criteria(self) -> bool:
        ...
        try:
            ...
            self.log_check(
                "0% Supabase Availability",
                passed,
                f"Analysis completed with Supabase disabled: {passed}"
            )
        finally:
        ...
        self.log_check(
            "Timeout Rate",
            passed,
            f"Timeout rate: {timeout_rate:.1%} (expected: <10%, operations: {health.total_operations})"
        )
        ...
        result, _ = await self.cache_service.get_or_execute(
            ticker="QUICK",
            asset_class="stock",
            execute_fn=quick_execute
        )
        ...
        self.log_check(
            ...
            passed,
            f"Operation completed in {elapsed:.3f}s (expected: <1.0s)"
        )
        ...
        self.log_check(
            "Logging Methods",
            passed,
            f"Client has logging methods: {passed}"
        )
...
async def main():
    ...
    parser.add_argument(
        ...
        type=int,
        choices=[1, 2],
        help="Deployment phase to validate (1 or 2)"
    )
    parser.add_argument(
        "--validate-all",
        action="store_true",
