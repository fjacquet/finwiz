...
class FinWizArchitectureValidator:
    ...
    def _validate_deep_analysis_result_schema(self):
        ...
        self._add_result(
            ValidationResult(
                check_name="DeepAnalysisResult schema",
                passed=passed,
                message="DeepAnalysisResult has all required fields" if passed else "DeepAnalysisResult missing required fields",
                requirement_refs=["1.5"],
                remediation="Add missing fields and extra='forbid' to DeepAnalysisResult" if not passed else None,
                details=details,
            )
        )
    ...
    def _validate_flow_sequence(self):
        ...
        if '@listen("validate_data_integration")' in content and "def check_portfolio" in content:
            pass
        else:
            passed = False
        ...
        if '@listen("check_portfolio")' in content and "def analyze_and_update_portfolio" in content:
            pass
        else:
            passed = False
    ...
    def _validate_atomic_operations(self):
        ...
        if has_method:
            ...
            if method_start != -1:
                ...
                has_update = "update" in method_content.lower() or "portfolio_review" in method_content.lower()
        ...
        self._add_result(
            ValidationResult(
                check_name="Atomic operations",
                passed=passed,
                message="analyze_and_update_portfolio is atomic" if passed else "analyze_and_update_portfolio is not atomic",
                requirement_refs=["2.8"],
                remediation="Consolidate deep analysis, alternatives, and update into single method" if not passed else None,
                details=details,
            )
        )
    ...
    def _validate_discovery_task_descriptions(self):
        ...
        crews = [("stock_crew", "1.6"), ("etf_crew", "1.7"), ("crypto_crew", "1.8")]
        ...
        for crew_name, req_ref in crews:
            ...
            self._add_result(
                ValidationResult(
                    check_name=f"{crew_name} task description",
                    passed=passed,
                    message=f"{crew_name} has correct task description" if passed else f"{crew_name} missing 'top 10' language",
                    requirement_refs=[req_ref],
                    remediation=f"Add 'screen and identify top 10' to {crew_name} task descriptions" if not passed else None,
                    details=details,
                )
            )
    ...
    def _validate_html_generation(self):
        ...
        for py_file in py_files:
            ...
            generates_html = (
                ("<html" in content.lower() and ('f"' in content or "f'" in content))
                or ("<!doctype html>" in content.lower())
                or ("<div" in content and ('f"' in content or "f'" in content))
                or ("<table" in content and ('f"' in content or "f'" in content))
                or ("<body" in content and ('f"' in content or "f'" in content))
            )
            ...
            if generates_html:
                ...
                uses_bs4 = "from bs4 import" in content or "import bs4" in content or "BeautifulSoup" in content
    ...
    def _validate_report_crew_tools(self):
        ...
        self._add_result(
            ValidationResult(
                check_name="ReportCrew tools",
                passed=passed,
                message="ReportCrew has empty tools and @final_reporter" if passed else "ReportCrew configuration incorrect",
                requirement_refs=["6.3", "6.4", "6.5", "6.6"],
...
"""
        ...
        for result in self.results:
            ...
            elif "Flow" in result.check_name or "Atomic" in result.check_name or "Listener" in result.check_name:
