...
class MetricsMonitor:
    ...
    def check_alerts(self, sample: dict):
        ...
        if not supabase["available"]:
            alert = {
                "timestamp": sample["timestamp"],
                "level": "WARNING",
                "message": "Supabase is unavailable - caching disabled",
                "metric": "availability",
                "value": False
            }
            self.alerts.append(alert)
    ...
    def print_sample(self, sample: dict):
        ...
        logger.info(f"  Operations: {supabase['total_operations']} (Success: {supabase['successful_operations']}, Failed: {supabase['failed_operations']}, Timeouts: {supabase['timeout_count']})")
                ...
                for line in f:
                    ...
                    if match:
                        cache_samples.append({
                            ...
                            "hit_rate": float(match.group(3)) / 100,
...
async def main():
    ...
    parser.add_argument(
        "--report",
        type=str,
        help="Generate report from log file"
    )
